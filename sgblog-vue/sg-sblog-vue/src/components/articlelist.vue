    ...
    <el-row class="sharelistBox">
        <el-col :span="24" class="s-item tcommonBox" v-for="(item,index) in articleList" :key="'article'+index">
            <span class="s-round-date">
                <span class="month" v-if="showInitDate(item.createTime,'month')" v-html="showInitDate(item.createTime,'month')+'月'"></span>
              <span  class="month" v-else>10月</span>
                <span class="day" v-if="showInitDate(item.createTime,'date')" v-html="showInitDate(item.createTime,'date')"></span>
              <span class="day" v-else>01</span>
            </span>
            <header>
                <h1>
                ...
                <h2>
                    ...
                    <i class="fa fa-fw fa-clock-o"></i><span v-if="showInitDate(item.createTime,'month')">{{showInitDate(item.createTime,'newDate')}}</span>
                  <span v-else>时间异常</span>•
        ...
        methods: { //事件处理器
            ...
            getList(){
                articleList(this.queryParams).then((response)=>{
                  ...
                  console.log('addd'+this.articleList)
