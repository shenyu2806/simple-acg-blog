...
import {getToken} from "../utils/auth";
    export default {
        ...
        methods: {
          getToken, //事件处理器
            ...
            saveInfoFun: function(){//保存编辑的用户信息
              ...
              const { head_start, ...userInfo } = that.userInfoObj;

              savaUserInfo(userInfo).then((response)=>{//保存信息接口，返回展示页
            ...
            routeChange: function(){//展示页面信息
                ...
                if(localStorage.getItem('userInfo')){
                    ...
                    getUserInfo(that.userId).then((response)=>{
                      ...
                      console.log('that.userInfoObj--------------',that.userInfoObj)
                      debugger
        ...
        created() { //生命周期函数
            this.routeChange();
            this.uploadURL = store.state.baseURL+'file/upload'
        }
    }
</script>
